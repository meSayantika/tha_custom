...
@Component({
  selector: "app-hotel-create-voice",
  templateUrl: "./hotel-create-voice.component.html",
  styleUrls: ["./hotel-create-voice.component.css"],
})
export class HotelCreateVoiceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataServe.global_service(0, "/voice", null).subscribe((data) => {
      console.log(data);
      this.voices = data;
  ...
  _filter(value: any) {
    ...
    console.log(
      this.filteredOptions1.filter((dt: any) => {
        dt.langName.match(filterValue);
        // console.log(dt.langName)
      })
    );
  ...
  getCat() {
    ...
    this.dataServe
      .global_service(0, "/cust_menu_list", `hotel_id=${this.hotel_id}`)
      .subscribe(
        (data: any) => {
  ...
  getVoice(v: any) {
    this.dataServe
      .global_service(0, "/pc_voice", `hotel_id=${this.hotel_id}&id=${this.id}`)
      .subscribe(
        (data) => {
          console.log(data);
          ...
          this.audFilePlay =
            environment.api_url + "/" + this.getvoiceData.msg[0].sound_path;
          this.audFilePlay_old =
            environment.api_url + "/" + this.getvoiceData.msg[0].sound_path;
  ...
  generateVoice() {
    ...
    this.http
      .post(
        "https://www.texttovoice.online/api/ConvertText.php",
        body.toString(),
        options
      )
      .subscribe(
  ...
  submitAud() {
    ...
    this.dataServe.global_service(1, "/pc_voice", dt).subscribe(
      (data) => {
        console.log(data);
        this.postVoiceData = data;
        if (this.postVoiceData.suc > 0) this.msg.successMsg("SS");
  ...
  go_prev_page() {
    this.router
      .navigate(["hotel/add-voice", btoa(this.hotel_id)])
      .catch((data) => {
  ...
  viewInfo() {
    this.dataServe.showInfo.next({
      data: "hotel_create_voice",
      flag: true,
      heading: "Create Voice",
    });
  }
}
