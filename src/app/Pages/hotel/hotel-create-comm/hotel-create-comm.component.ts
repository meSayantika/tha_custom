...
@Component({
  selector: 'app-hotel-create-comm',
  templateUrl: './hotel-create-comm.component.html',
  styleUrls: ['./hotel-create-comm.component.css']
})
export class HotelCreateCommComponent implements OnInit {
  msgForm!: FormGroup;
  hide=false;
  scale = 1;
  restrictTitle=0
  ...
  ngOnInit() {
    this.dataServe.local_service(environment.langUrl).subscribe(data=>{
      console.log(data)
      this.languages=data
  ...
  sendMessage(){
    ...
    console.log(this.selectedVIPs)
      ...
      formData.append("message_center_type",this.routeFlag==0?this.m.msg_type.value:(this.routeFlag==1?'R':'O'))
  ...
  getMessage(){
    ...
    this.restrictTitle=this.getmsgData.msg[0]?.msg_title.length
    this.restrictSubTitle=this.getmsgData.msg[0].msg_subtitle?.length
    this.restrictMsgBody=this.getmsgData.msg[0]?.msg_body.length
    // this.bitly_url
  ...
  if(this.aud=='1S'){
    const dialogRef = this.dialog.open(DialogBoxComponent, {
      ...
      data: { flag: 'singleGuest', content:{'hotel_id':this.hotel_id}}
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result)
      this.selectedVIPs=result
