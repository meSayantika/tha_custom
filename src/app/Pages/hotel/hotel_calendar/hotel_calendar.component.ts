...
@Component({
  selector: 'app-hotel_calendar',
  templateUrl: './hotel_calendar.component.html',
  styleUrls: ['./hotel_calendar.component.css'],
  providers:[DatePipe]
})
export class Hotel_calendarComponent implements OnInit {
 ...
 ngOnInit(){
  ...
  go_to_message(v:any){
    ...
    this.router.navigate(['/hotel/hotel_create_calendar',btoa(this.hotel_id),btoa(v)])
  ...
  getMessage(){
    this.dataServe.global_service(0,'/calendar',`hotel_id=${this.hotel_id}&srv_res_flag=${this.category}&srv_res_id=${this.rest_serve}`).subscribe(data=>{
      console.log(data)
      this.msgData=data
  ...
  openSearch(){
    ...
    dialogRef.afterClosed().subscribe(result => {
     console.log(result)
     this.dataServe.global_service(0,'/calendar',`hotel_id=${this.hotel_id}&frm=${result.from}&to=${result.to}&srv_res_flag=${this.category}&srv_res_id=${this.rest_serve}`).subscribe(data=>{
      console.log(data)
      this.msgData=data
      this.putdata(this.msgData.msg)
    } ,error=>{this.msg.globalError(error.status+' '+error.statusText+' in '+error.url)})
