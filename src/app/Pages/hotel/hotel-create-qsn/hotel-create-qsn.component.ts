...
@Component({
  selector: 'app-hotel-create-qsn',
  templateUrl: './hotel-create-qsn.component.html',
  styleUrls: ['./hotel-create-qsn.component.css']
})
export class HotelCreateQsnComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.servedeptInfo=this.formBuilder.group({
      ...
      rs_bar_menu: ['', [Validators.required]],
  ...
  this.servedeptInfo.patchValue({
    ...
    rs_break_menu: this.qsetData.msg[0].service[1].breakfast=='Y'??'Y',
      ...
      rs_bar_menu: this.qsetData.msg[0].service[1].bar_menu=='Y'??'Y',
  ...
  sendProposal(){
    ...
    //  this.dataServe.global_service(1,'/registration',dt).subscribe(data=>{
      ...
      service_dtls.push({
        ...
        bar_menu:this.s.rs_bar_menu.value??'Y',
...
go_prev_page(){
  this.router.navigate(['hotel/hotel_qsn',btoa(this.hotel_id)]).catch(data=>{
