...
@Component({
  selector: 'app-hotel-create-prop',
  templateUrl: './hotel-create-prop.component.html',
  styleUrls: ['./hotel-create-prop.component.css']
})
export class HotelCreatePropComponent implements OnInit {
  id: any;
  hotel_id: any;
  flag: any;
  eFlag: any;
  ...
  ngOnInit() {
    ...
    this.addFee()
    console.log('Flag', this.eFlag);
    
    this.showHtml=this.eFlag=='E' || this.eFlag=='S'?true:false
      ...
      if(result==1)
       {
        ...
        this.dataServe.global_service(1,'/update_proposal',dt).subscribe(data=>{
          ...
          if(this.updatePropData.suc>0){
           ...
           if(v=='A'){
            this.router.navigate(['/hotel/payment_page',btoa(this.id.toString()),btoa(this.hotel_id),btoa('U'),btoa('S')]).catch(data=>this.msg.globalError(data))
           }
          //  console.log(v);
  ...
  go_prev_page(){
    this.router.navigate(['hotel/hotel_proposal',btoa(this.hotel_id)]).catch(data=>{
