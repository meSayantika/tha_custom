...
@Component({
  selector: 'app-create-group-notif',
  templateUrl: './create-group-notif.component.html',
  styleUrls: ['./create-group-notif.component.css']
})
export class CreateGroupNotifComponent implements OnInit {
  msgForm!: FormGroup;
  hide=false;
  scale = 1;
  restrictTitle=0
  ...
  getMember(){
      ...
      this.dataServe.global_service(0,'/guest_user_dt',`hotel_id=${this.hotel_id}&group_id=${localStorage.getItem('group_emp_id')}&flag=N`).subscribe(data=>{console.log(data)
       ...
       this.groupMemberData=this.groupMemberData.msg
        },error=>{this.msg.globalError(error.status+' '+error.statusText+' in '+error.url)})
  ...
  sendMessage(){
    ...
    console.log(this.m.audience.value)
      const formData = new FormData()
      formData.append("id",this.id,)
      formData.append("hotel_id",this.hotel_id)
      formData.append("message_center_type",'G')
      formData.append("audience",this.m.audience.value)
      formData.append("user_dt",JSON.stringify(this.m.audience.value))
  ...
  go_prev_page(){
    ...
    this.router.navigate(['main/admin/group_home',btoa(this.hotel_id)]).catch(data=>{
      this.msg.globalError(data);
    })
  }
  }
