...
@Component({
  selector: 'app-mng-communications',
  templateUrl: './mng-communications.component.html',
  styleUrls: ['./mng-communications.component.css']
})
export class MngCommunicationsComponent implements OnInit {
 ...
 ngOnInit(){
  ...
  go_to_message(v:any){
    ...
    this.router.navigate(['/main/admin/create_notification',btoa(this.hotel_id),btoa(v)])
    else
    this.router.navigate(['/main/admin/notif_placeholder',btoa(this.hotel_id),btoa(v),localStorage.getItem('mngFlag')])

  }
  getMessage(){
    this.dataServe.global_service(0,'/message_center',`hotel_id=${this.hotel_id}&user=${localStorage.getItem('email')}`).subscribe(data=>{
      console.log(data)
      this.msgData=data
