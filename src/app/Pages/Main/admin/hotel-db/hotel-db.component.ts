...
@Component({
  selector: 'app-hotel-db',
  templateUrl: './hotel-db.component.html',
  styleUrls: ['./hotel-db.component.css']
})
export class HotelDbComponent implements OnInit {
  displayedColumns: string[] = ['id', 'restaurant_name', 'date_enquiry','country','edit'];
  ...
  // localStorage.setItem('Restaurant_email',this.logData.msg.email_id);
    window.open(environment.routeUrl+'main/admin/hotel_db_data/'+encodeURIComponent(btoa(v.restaurant_name))+'/'+btoa(v.id))
    
  }
  fetchdata() { //fetching the restaurant record
    //Call APi
    this.dataServe.global_service(0,'/res_dtls_custom',`id=0`).subscribe(data => {
      console.log(data)
      this.userData = data
      // this.userData = this.userData.msg;
      // this.show_spinner=true;
      this.userData=this.userData.msg.filter((e:any)=>e.final_pay!=null || e.first_pay!=null)
      console.log(this.userData)
      this.putdata(this.userData)
