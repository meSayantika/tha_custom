...
@Component({
  selector: 'app-entertainment-fun-create',
  templateUrl: './entertainment-fun-create.component.html',
  styleUrls: ['./entertainment-fun-create.component.css']
})
export class EntertainmentFunCreateComponent implements OnInit {
  msgForm!: FormGroup;
  hide=false;
  scale = 1;
  restrictTitle=0
  ...
  sendMessage(){
    ...
    console.log(this.selectedVIPs)
      ...
      formData.append("bg_color",this.msgForm.controls.bgCol.value)
      formData.append("user",this.msgForm.controls.txtCol.value)
      console.log(formData.getAll("user_dt"))
      debugger;
     this.dataServe.global_service(1,'/fun_directory_calendar',formData).subscribe(data=>{
      console.log(data)
  ...
  getMessage(){
    this.dataServe.global_service(0,'/fun_directory_calendar',`id=${this.id}`).subscribe(data=>{console.log(data)
    ...
    this.msgForm.patchValue({
      ...
      bgCol:this.getmsgData.msg[0].bg_color,
      txtCol:this.getmsgData.msg[0].txt_color,
