...
@Component({
  selector: 'app-mng-create-notif',
  templateUrl: './mng-create-notif.component.html',
  styleUrls: ['./mng-create-notif.component.css']
})
export class MngCreateNotifComponent implements OnInit {
  msgForm!: FormGroup;
  hide=false;
  scale = 1;
  restrictTitle=0
  ...
  getAudData:any
  filteredNames:any
  dep_name=localStorage.getItem('name')
  msg_type=localStorage.getItem('msg_type')
  sendTimeVal=''
  audType=''
  ...
  getAudience(){
    if(localStorage.getItem('mngFlag')=='E')
    // { this.dataServe.global_service(0,'/emp_dtls',`hotel_id=${this.hotel_id}`).subscribe(data=>{console.log(data)
      { this.dataServe.global_service(0,'/emp_sche_list',`hotel_id=${this.hotel_id}`).subscribe(data=>{console.log(data)
      this.getAudData=data
      this.filteredNames=this.getAudData.msg.filter((e:any)=>e.emp_dept_id==localStorage.getItem('dept') && e.mobile_no!=localStorage.getItem('phone'))
  ...
  sendMessage(){
    ...
    console.log(this.selectedVIPs)
      ...
      formData.append("message_center_type",localStorage.getItem('mngFlag')=='E'?'E':'G')
...
,error=>{this.msg.globalError(error.status+' '+error.statusText+' in '+error.url)}
  ...
  profile(){
    const dialogRef = this.dialog.open(DialogBoxComponent, {
      width: '460px',
      data: {flag:'profile',content:null}
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
